...
export default function(data, hdbData, keyword) {
  function generateHDBSubplot(hdbData) {
  ...
  if (hdbData != null) {
    var hdbChart = generateHDBSubplot(hdbData)
    const totalHDBData = hdbData.filter(d => d.demographic === 'Total' && d.housing != 'Total')
  ...
  langSelect.addEventListener('change', (event) => {
    ...
    const totalNarrative = `A majority (${util.formatPercent(percentage)}) 
    ...
    if (hdbData != null) {
      ...
      const hdbNarrative = `Among the ${event.target.value} group HDB residents,
                            ...
                            the national average of ${util.formatPercent(totalHDBPercentage)}.`
      narrative += hdbNarrative
    }
